...
import { Plus, Mail, Phone, MapPin, Building2, User, Search, Edit, Trash2, Grid3X3, List, ArrowUpDown, ArrowUp, ArrowDown, Eye, MoreHorizontal, RefreshCw, Copy, XCircle } from "lucide-react";
import {
  DropdownMenu,
  DropdownMenuContent,
  DropdownMenuItem,
...
export default function Lessees() {
  ...
  const [cancelInviteLessee, setCancelInviteLessee] = useState<Lessee | null>(null);
  ...
  const resendInviteMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest("POST", `/api/lessees/${id}/resend-invite`),
    onSuccess: () => {
  ...
  const cancelInviteMutation = useMutation({
    mutationFn: (id: number) =>
      apiRequest("DELETE", `/api/lessees/${id}/invite`),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["/api/lessees"] });
      setCancelInviteLessee(null);
  ...
  async function copyInviteLink(id: number) {
    try {
      const res = await apiRequest("GET", `/api/lessees/${id}/invite-link`);
  ...
  return (
    <>
      <Helmet>
        <title>Flight Schools — AeroLease Wise</title>
      </Helmet>

      <div className="flex justify-between items-center mb-6">
        <div>
          <h1 className="text-[32px] font-bold tracking-tight text-[#1e293b]">Flight Schools</h1>
      ...
      {isLoading ? (
        ...
        viewMode === 'grid' ? (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {filteredAndSortedLessees.map((lessee) => (
              <Card key={lessee.id} className="rounded-[24px] border-[#f1f5f9] shadow-sm bg-white hover:shadow-md transition-shadow overflow-hidden">
                <CardContent className="p-6">
                  <div className="flex justify-between items-start">
                    <div>
                      <h3 className="font-bold text-lg text-[#1e293b]">{lessee.name}</h3>
                        ...
                        {lessee.portalStatus === "invited" && (
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="start" onClick={(e) => e.stopPropagation()}>
                              <DropdownMenuItem onClick={() => resendInviteMutation.mutate(lessee.id)}>
                                <RefreshCw className="h-4 w-4 mr-2 text-blue-600" />
                                Resend Invite
                              </DropdownMenuItem>
                              <DropdownMenuItem onClick={() => copyInviteLink(lessee.id)}>
                                <Copy className="h-4 w-4 mr-2 text-gray-600" />
                                Copy Invite Link
                              </DropdownMenuItem>
                              <DropdownMenuSeparator />
                              <DropdownMenuItem className="text-red-600 focus:text-red-600" onClick={() => setCancelInviteLessee(lessee)}>
                  ...
                  <div className="mt-4 space-y-2">
                    {lessee.contactPerson && (
                      <div className="flex items-center text-sm">
                        <User className="h-4 w-4 mr-2 text-[#64748b]" />
                        <span className="text-[#475569]">{lessee.contactPerson}</span>
                      </div>
                    )}
                    ...
                    {lessee.phone && (
                      <div className="flex items-center text-sm">
                        ...
                        <a href={`tel:${lessee.phone}`} className="text-[#475569]">
                          {lessee.phone}
                        </a>
                      </div>
                    )}
                    ...
                    {lessee.address && (
                      <div className="flex items-start text-sm">
                        ...
                        <span className="text-[#475569]">{lessee.address}</span>
                      </div>
                    )}
                  </div>
                  ...
                  {lessee.notes && (
                    <div className="mt-4 p-3 bg-white rounded-md border border-slate-100 text-sm">
                      <p className="font-medium text-[#475569] mb-1">Notes</p>
                      <p className="text-[#64748b]">{lessee.notes}</p>
                    </div>
                  )}
          ...
          <Card className="rounded-2xl border-[#f1f5f9] shadow-sm overflow-hidden bg-white">
            <Table>
              ...
              <TableBody>
                {filteredAndSortedLessees.map((lessee) => (
                  <TableRow key={lessee.id} className="hover:bg-[#f8fafc] transition-colors border-b border-[#f1f5f9]">
                    <TableCell className="font-medium text-[#1e293b]">{lessee.name}</TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      {lessee.phone ? (
                        <a href={`tel:${lessee.phone}`} className="text-[#475569] hover:underline">
                          {lessee.phone}
                        </a>
                      ) : (
        ...
        <div className="text-center py-12">
          <div className="mx-auto h-16 w-16 rounded-full bg-[#f8fafc] flex items-center justify-center mb-4">
            <Building2 className="h-8 w-8 text-[#cbd5e1]" />
          </div>
          <h3 className="text-lg font-medium text-[#1e293b] mb-1">No flight schools found</h3>
      ...
      <AlertDialog open={!!cancelInviteLessee} onOpenChange={(open) => {
        if (!open) setCancelInviteLessee(null);
      }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Cancel Invite</AlertDialogTitle>
            <AlertDialogDescription>
              Are you sure you want to cancel the portal invite for "{cancelInviteLessee?.name}"? They will no longer be able to use the invite link.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
            <AlertDialogCancel>Cancel</AlertDialogCancel>
            <AlertDialogAction
              className="bg-red-600 text-white hover:bg-red-700"
              onClick={() => {
                if (cancelInviteLessee) {
                  cancelInviteMutation.mutate(cancelInviteLessee.id);
                }
              }}
            >
